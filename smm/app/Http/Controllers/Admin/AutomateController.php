...
class AutomateController extends \App\Http\Controllers\Controller
{
    ...
    public function sendOrdersIndexData()
    {
        ...
        return datatables()->of($orders)->addColumn('bulk', function($order) {
			...
			return '<input type=\'checkbox\' ' . $disabled . ' class=\'input-sm row-checkbox\' name=\'order_id[' . $order->id . ']\' value=\'' . $order->id . '\'>';})->addColumn("api", "admin.automate.send-orders-api-select")->addColumn("action", "admin.automate.send-orders-action-buttons")->editColumn("link", function($order)
        {
